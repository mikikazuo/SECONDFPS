...
void mob::move(){
	...
	vec3 toplayer_dir = vec3(sinf(angles.x), 0, cosf(angles.x));//vec3((get_player()->position.x-position.x), (get_player()->position.y-position.y), (get_player()->position.z-position.z));
	...
	if(!vsinfo.findplayer){
		...
		if(!this->hitmap&&(mobhitobj.sethitcheck(get_mapobj()->get_objnum(),get_mapobj()->get_obj(),sampposition,radi)/*||
				obhitobj.sethitcheck(WALLMAX,get_allplayerwall()[0],sampposition,radi)*/)	){
			angles.x -= M_PI;
			if(angles.x < -M_PI)
				angles.x += M_PI * 2;
			else if(angles.x > M_PI)
