...
void Start::Update() {
	...
	case 0:{
		if(get_mousebutton_count(LEFT_BUTTON)==2){
			if(red.leftup.x<get_mouseinfo().x&&get_mouseinfo().x<red.leftup.x+red.width&&
					red.leftup.y<get_mouseinfo().y&&get_mouseinfo().y<red.leftup.y+red.height	){
				s_flag = 1;
				//get_SceneMgr().ChangeScene(eScene_Game);	/*シーンを入れ込む*/
				set_myteam(RedTeam);
				set_mousespeed(0.08+0.02*(smoothvalue-2));
			}else if(blue.leftup.x<get_mouseinfo().x&&get_mouseinfo().x<blue.leftup.x+blue.width&&
					blue.leftup.y<get_mouseinfo().y&&get_mouseinfo().y<blue.leftup.y+blue.height){
				s_flag = 1;
				//get_SceneMgr().ChangeScene(eScene_Game);	/*シーンを入れ込む*/
				set_myteam(BlueTeam);
				set_mousespeed(0.08+0.02*(smoothvalue-2));
	...
	case 1:{
		...
		if(100<get_mouseinfo().x&&get_mouseinfo().x<400&&
			0<get_mouseinfo().y&&get_mouseinfo().y<400 ){
			ro_flag = 0;
			puts("範囲");
		}
		else ro_flag = -1;
...
void Start::Draw() {
	...
	case 1:{
		...
		if(ro_flag == 0) {glRotatef(angle,0, 1, 0);}
